...
class ThreeBandEQ    : public Component
{
public:
    ThreeBandEQ()
	{
...
float* process(float left, float right){
	...
	output[0] = clipper1.process(sin(float_Pi*(highMidRatio*midh + midMidRatio*midm + lowMidRatio*midl + highSideRatio*lefth + midSideRatio*leftm + lowSideRatio*leftl)/2.0));
	output[1] = clipper2.process(sin(float_Pi*(highMidRatio*midh + midMidRatio*midm + lowMidRatio*midl + highSideRatio*righth + midSideRatio*rightm + lowSideRatio*rightl)/2.0));
...
/*
    ...
    */
	...
	output[0] = sin(float_Pi/2.0*(highMidGain*(midh) + midMidGain * (midm) + lowMidGain * (midl))*midGain + (highSideGain * (lefth) + midSideGain * (leftm) + lowSideGain * (leftl))*sideGain);
	output[1] = sin(float_Pi/2.0*(highMidGain*(midh) + midMidGain * (midm) + lowMidGain * (midl))*midGain + (highSideGain * (righth) + midSideGain * (rightm) + lowSideGain * (rightl))*sideGain);
